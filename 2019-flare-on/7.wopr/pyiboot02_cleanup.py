...
# Size of source mod 2**32: 272 bytes https://tool.lu › pyc
"""
...
from ctypes import *
print('LOADING...')
BOUNCE = pkgutil.get_data('try_this', 'key')
...
"""
with open("try_this/key", "rb") as f:
    BOUNCE = f.read()
"""
for i in range(256):
    new_bounce = bytes([i]) + BOUNCE
    decode = fire(eye(__doc__.encode()), new_bounce)
    try:
        source_code = lzma.decompress(decode)
        break
    except Exception as e:
        pass
with open("worp.py", "w") as f:
    f.write(source_code.decode())